...
def train_pytorch_model(X_train, y_train, X_valid, y_valid, input_dim, 
    ...
    X_valid_tensor = torch.FloatTensor(X_valid.values).to(device)
    y_valid_tensor = torch.FloatTensor(y_valid.values).to(device)
    ...
    model = TwoDtoThreeDModel(input_dim=input_dim).to(device)
...
def save_model(model, model_type, model_name=None):
    """Save model to disk"""
...
def load_model(model_type, model_name):
    ...
    if model_type == 'rf':
        ...
        print(f"\nRandomForest model loaded from: {model_path}")
        return model
        ...
        print(f"\nPyTorch model loaded from: {model_path}")
        return model
